...
abstract class BaseKeyframeAnimation<K, A> {
  interface AnimationListener<A> {
    void onValueChanged(A progress);
  }
  ...
  private Keyframe<K> getCurrentKeyframe() {
    ...
    if (cachedKeyframe != null && progress >= cachedKeyframe.getStartProgress() &&
        progress <= cachedKeyframe.getEndProgress()) {
      return cachedKeyframe;
    }

    int i = 0;
    Keyframe<K> lastKeyframe = keyframes.get(keyframes.size() - 1);
    Keyframe<K> keyframe = keyframes.get(0);
    if (progress < keyframe.getStartProgress()) {
      return keyframe;
    } else if (progress > lastKeyframe.getEndProgress()) {
      return lastKeyframe;
    }
  ...
  private float getCurrentKeyframeProgress() {
    ...
    return keyframe.timingFunction.getInterpolation(progressIntoFrame / keyframeProgress);
  }
